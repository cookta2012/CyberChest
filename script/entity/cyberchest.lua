...
--luacheck: ignore global, ignore cyberchest
--luacheck: ignore game defines
--luacheck: ignore Table

local Table = require("__stdlib__/stdlib/utils/table");
local finder = require("__CyberChest__/utils/recipe_finder")

--local LOGGER = require('__stdlib__/stdlib/misc/logger').new("logg")
--Remember to use game.surfaces['nauvis'] instead of game when dealing with position stuff like when doing find_entities

local asm_sides = {left = 1, right = 2, top = 3, bottom = 4}
...
function cyberchest.get_recipes(self)
	...
	for i = 1, self.requester_slots do
		local item = self.get_request_slot(i)
		if item then
			...
			if recipe then
				self.orders[k] = {
					target_stack = item,
					recipe_name = recipe,
					result_amount = finder.Result_Count(self.assembler.force.recipes[recipe], item.name),
					stack_size = game.item_prototypes[item.name].stack_size,
					ingredients = finder.ingredients_to_simplestack(recipe),
...
function cyberchest.get_recipe_name_for(self, item_name)
	...
	local recipe_name = ""
	repeat
		recipe_name = finder.Get_Recipe_For(item_name, exclude_list)
...
function cyberchest.recipe_is_good(self, recipe_name)
	...
	--try set recipe
	self.assembler.set_recipe(recipe_name)
	if not self.assembler.get_recipe() then
...
function cyberchest.initialize_assembler(self)
	...
	if self.assembler.get_recipe() and self.assembler.get_recipe().name == recipe_name then --skip
	...
	for _,item_stack in pairs(order.ingredients) do
		...
		if asm_in_count < desirable_stack.count then
			...
			if transfer_stack.count > 0 then
				finder.stack_transfer(self.inv, transfer_stack, self.assembler)
			end
		end
	end
	self:restore_bar()
...
function cyberchest.assembler_clear_inventory(self, asm_inv)
	...
	for name,count in pairs(item_map) do
		if not finder.stack_transfer(asm_inv, {name = name, count = count}, self.inv) then
			self:restore_bar()
			return false
		end
	end
